...
Notes let you attach additional context, updates, or commentary to a workspace item without changing the item itself. They're useful for capturing what's happening *around* a piece of work — progress, blockers, decisions, follow-ups — so the history stays with the item instead of living in chat, email, or meeting recaps.

## How Notes work

Notes can be added to any workspace item that supports them, including:
...
Every workspace item has a **note icon** on its right side, just to the left of the three-button menu (Edit, History, Archive, Delete).
...
1. Locate the item you want to add a Note to.
2. Click the **note icon** on the right side of the item.
...
- Notes can't be added to Objectives, Core Values, Org Chart roles, meetings, or Agendas — only to the workspace items listed above.

...
1. Open the item you want to add a link from, either by creating a new one or editing an existing one.
2. Find the **link dropdown menu**.
3. Browse the available items, grouped by category, or search for a specific one.
4. Select the item you want to link. The link is added immediately.
5. Save the item.
...
1. Open the item in edit view.
2. Open the link dropdown menu.
3. Select the linked item again to remove the link.
4. Save the item.
...
- Anyone with access to a team can add or remove links on its items.
...
- Most links are navigational only — they help you jump between related items but don't change behavior. The exception is the Rock-to-Objective link, which contributes to Objective progress automatically.
